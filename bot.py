...
def salvar_questao(user_id, dados, tipo, ia):
    ...
    cur = conn.cursor()
    cur.execute("""
        insert into questoes (telegram_user_id, disciplina, assunto, nucleo, resumo, mnemonico, armadilha, tipo, ia_usada)
        values (%s, %s, %s, %s, %s, %s, %s, %s, %s)
    """, (str(user_id), dados["disciplina"], dados["assunto"], dados["nucleo"],
          dados["resumo"], dados["mnemonico"], dados["armadilha"], tipo, ia))
    conn.commit()
    cur.close()
    conn.close()
...
def buscar_questoes(user_id, disciplina=None, limit=5):
    ...
    if disciplina:
        ...
        """, (str(user_id), f"%{disciplina}%", limit))
    else:
        cur.execute("""
...
async def analisar_imagem(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    msg = await update.message.reply_text("🔍 Analisando a questão...")
    try:
        ...
        logging.exception("erro analisar_imagem")
        await msg.edit_text(f"❌ Erro ao analisar: {str(e)}")
...
async def analisar_texto(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logging.exception("erro analisar_texto")
        await msg.edit_text(f"❌ Erro ao analisar: {str(e)}")
...
async def salvar_errei(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logging.exception("erro salvar_errei")
...
async def salvar_favorita(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logging.exception("erro salvar_favorita")
        await update.message.reply_text(f"❌ Erro ao salvar: {str(e)}")

async def stats(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    try:
        qs = estatisticas(update.effective_user.id)
        if not qs:
        ...
        logging.exception("erro stats")
        await update.message.reply_text(f"❌ Erro: {str(e)}")

async def revisao(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    try:
        disciplina = " ".join(ctx.args) if ctx.args else None
        qs = buscar_questoes(update.effective_user.id, disciplina)
        if not qs:
        ...
        for q in qs:
            await update.message.reply_text(
                ...
                f"🧠 `{q['mnemonico']}`\n\n"
                f"⚠️ {q['armadilha']}\n\n"
                f"_{q['created_at'].strftime('%d/%m/%Y')}_",
                parse_mode="Markdown"
            )
    except Exception as e:
        logging.exception("erro revisao")
        await update.message.reply_text(f"❌ Erro: {str(e)}")
